            ...
            target,
            {
                ...
                update : (function (index, dom, onlyDirty = true) {
                    ...
                    node.getAncestors().reverse().forEach(function (parent) {
                        if (parent.isLast()) {
                            html += `<i class="jstree-icon icon-flipped">&nbsp;</i>`;
                        } else {
                            html += `<i class="jstree-icon jstree-icon-v icon-flipped">&nbsp;</i>`;
                        }
                    });
                    let clss = 'leaf';
                    ...
                    if (node.isLast()) {
                        html += `<i class="jstree-icon jstree-icon-v2 jstree-icon-h icon-flipped"><span class="jstree-${clss}">&nbsp;</span></i>`;
                    } else {
                        html += `<i class="jstree-icon jstree-icon-v jstree-icon-h icon-flipped"><span class="jstree-${clss}">&nbsp;</span></i>`;
                    }
                    clss = this.getState(node, "selected", false) ? 'jstree-selected' : '';
                    html += `<a class="jstree-node-text ${clss}" href="#"><i class="jstree-icon jstree-node-icon icon-flipped">&nbsp;</i> ${node.data.text}</a>`;
...
export function dummy(levels, nodes) {
    console.log("levels, nodes: ", levels, nodes)
    var data = [], i;
    var c = 0;
...
//     .openAll();
    ...
    // console.log("jsTree instance: ", instance);
